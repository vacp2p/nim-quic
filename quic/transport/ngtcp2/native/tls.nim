...
proc init*(
  ...
  if sslCtx.isNil:
    return err("could not instantiate ssl context")
  ...
  if configureResult != 0:
    return err("could not configure context")
  ...
  if isServer:
    ...
    if (
      ...
      return err("could not set supported algorithm list")
  ...
  if key.len != 0 and certificate.len != 0:
    ...
    if SSL_CTX_use_certificate(sslCtx, cert) != BORINGSSL_OK:
      return err("could not use certificate")

    if SSL_CTX_use_PrivateKey(sslCtx, pkey) != BORINGSSL_OK:
      return err("could not use private key")

    if SSL_CTX_check_private_key(sslCtx) != BORINGSSL_OK:
      return err("cant use private key with certificate")
